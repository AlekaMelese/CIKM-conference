...
WORKFLOW:
1. Fine-tuned Gemma-2-9B-IT model (Phase 1) - Trained on narrative summaries
...
class NarrativeRAGConfig:
    ...
    # MATCH GEMMA BASELINE EXACTLY - don't copy from other models!
    # Key insight: Baseline does NOT use min_new_tokens - let model stop naturally
    MAX_SEQ_LENGTH = 4096   # BASELINE
    MAX_NEW_TOKENS = 768    # BASELINE: default (uses 1024 for long inputs)
    MIN_NEW_TOKENS = 0      # BASELINE: NOT USED - let model stop naturally!
    TEMPERATURE = 0.6       # BASELINE: 0.6
    TOP_P = 0.92            # BASELINE: 0.92
...
class NarrativeRAGInference:
    ...
    def __init__(self):
        ...
        print("Fine-tuned Gemma-2-9B-IT + Narrative Style Retrieval")
    ...
    def _load_finetuned_model(self):
        """Load Phase 1 fine-tuned NARRATIVE Gemma model"""
    ...
    def build_narrative_prompt(self, input_note: str, retrieved_cases: List[Dict] = None) -> str:
        ...
        """
        # BASELINE PROMPT - exact same format as gemma_narrative_finetune.py
...
"""
    ...
    def clean_narrative_output(self, text: str) -> str:
        ...
        Baseline (gemma_narrative_generation.py lines 132-139) ONLY does:
        1. Check if generation was cut off mid-sentence
        2. If so, keep only complete sentences

        NO other cleaning! Don't remove content, don't return empty string!
        """
        if not text:
            return text
        ...
        if text and text[-1] not in '.!?':
            # Find last complete sentence (matching baseline lines 132-139)
    ...
    def generate_narrative_summary(self, input_note: str, retrieved_cases: List[Dict] = None) -> str:
        ...
        # DYNAMIC max_tokens calculation (MATCHING BASELINE gemma_narrative_generation.py)
        # Baseline uses SIMPLE logic: 1024 for long inputs, 768 for others
        ...
        # Use 1024 for long inputs (>3000 tokens), otherwise use MAX_NEW_TOKENS (1024)
        # This matches gemma_narrative_generation.py lines 107-111
        ...
        # Generate with EXACT BASELINE parameters from gemma_narrative_generation.py
        # Note: Baseline does NOT use min_new_tokens - let model stop naturally
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                # min_new_tokens NOT USED - matching baseline behavior
