...
class NarrativeRAGEvaluator:
    ...
    def __init__(self):
        ...
        # Medical entity patterns (MATCHING gemma_metrics.py exactly)
    ...
    def get_clinical_bert(self):
        """Lazy load ClinicalBERT model (MATCHING gemma_metrics.py)"""
    ...
    def get_nli_model(self):
        """Lazy load NLI model (MATCHING gemma_metrics.py)"""
    ...
    def extract_medical_entities(self, text: str):
        """Extract medical entities using regex patterns (MATCHING gemma_metrics.py)"""
        entities = {
            'medications': set(),
            'diagnoses': set(),
            'procedures': set()
    ...
    def compute_hallucination_metrics(self, generated, input_text):
        """
        Compute hallucination metrics (MATCHING Phase 1 gemma_metrics.py exactly)
    ...
    def compute_entity_metrics(self, generated, reference):
        """
        Compute entity extraction metrics (MATCHING gemma_metrics.py)
    ...
    def compute_factual_consistency(self, generated, input_text):
        """
        Calculate factual consistency using NLI (MATCHING gemma_metrics.py)
    ...
    def evaluate_sample(self, result):
        ...
        # This matches gemma_metrics.py behavior
    ...
    def create_summary_report(self, aggregated):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"**Model**: Fine-tuned Gemma-2-9B-IT (Narrative) + Hybrid RAG\n\n")
            f.write(f"**Date**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            f.write(f"**Test Samples**: {len(self.rag_results)}\n\n")
            f.write("=" * 80 + "\n\n")

            f.write("## System Configuration\n\n")
            f.write(f"- **Base Model**: Gemma-2-9B-IT (narrative fine-tuned)\n")
