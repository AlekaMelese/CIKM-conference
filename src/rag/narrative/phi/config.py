...
1. PEFT (Phase 1): Fine-tuned Phi-3-medium-4k-instruct model provides clinical domain adaptation
...
class RAGConfig:
    ...
    PROJECT_ROOT = BASE_DIR.parent.parent.parent.parent  # ./Llama
    ...
    # Phase 1 fine-tuned Phi model path
    PHASE1_MODEL_NAME = "unsloth/Phi-3-medium-4k-instruct-bnb-4bit"
    ...
    # Model generation parameters (OPTIMIZED - same as phi_constrained_generation.py)
    MAX_SEQ_LENGTH = 6144  # Maximum sequence length for model
    MAX_NEW_TOKENS = 3072  # Maximum new tokens to generate
    TEMPERATURE = 0.4  # OPTIMIZED: 0.7 → 0.4 (more balanced, not too random)
    TOP_P = 0.95  # Nucleus sampling (already optimal)
    TOP_K = 100  # Top-k sampling
    REPETITION_PENALTY = 1.1  # OPTIMIZED: 1.15 → 1.1 (allow more lexical overlap)
...
"""
    ...
    def print_config(cls):
        ...
        print(f"\n🤖 MODELS")
        print(f"  Generator (PEFT): Fine-tuned Phi-3-medium-4k-instruct (Phase 1)")
...
# Alias for Narrative RAG (used by 4_evaluate_rag.py)
# NarrativeRAGConfig has the same paths but uses 'target' instead of 'structured_target'
class NarrativeRAGConfig(RAGConfig):
    """Configuration for Narrative RAG - extends RAGConfig with narrative-specific settings"""

    # Override retrieval field for narrative
    RETRIEVAL_FIELD = "target"  # Narrative paragraphs (not structured_target)

    # Output paths - use main outputs folder for narrative
    RAG_OUTPUTS_DIR = RAGConfig.BASE_DIR / "outputs"
    ...
    EVALUATION_RESULTS_PATH = RAG_OUTPUTS_DIR / "narrative_evaluation_results.json"
