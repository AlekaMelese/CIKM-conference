...
WORKFLOW:
1. Fine-tuned Mistral-7B-Instruct model (Phase 1) - Trained on narrative summaries
...
class NarrativeRAGConfig:
    ...
    # Baseline achieved: ROUGE1=0.272, BERTScore=0.724, avg 179 words
    # Key insight: RAG was generating too long (230 words), hurting ROUGE2/L
    MAX_SEQ_LENGTH = 4096   # BASELINE
    MAX_NEW_TOKENS = 512    # REDUCED: limit output length to match baseline ~179 words
    MIN_NEW_TOKENS = 200    # REDUCED: from 240 to allow shorter, more focused outputs
    TEMPERATURE = 0.5       # REDUCED: from 0.6 for more deterministic/precise outputs
    TOP_P = 0.9             # BASELINE: 0.9
    TOP_K = 50              # BASELINE: 50
    REPETITION_PENALTY = 1.05  # BASELINE: 1.05
    LENGTH_PENALTY = 1.0    # REDUCED: from 1.2 - don't encourage longer outputs
    NO_REPEAT_NGRAM_SIZE = 3  # BASELINE: 3
...
class NarrativeRAGInference:
    ...
    def __init__(self):
        ...
        print("Fine-tuned Mistral-7B-Instruct + Narrative Style Retrieval")
    ...
    def _load_finetuned_model(self):
        """Load Phase 1 fine-tuned NARRATIVE Mistral model"""
    ...
    def build_narrative_prompt(self, input_note: str, retrieved_cases: List[Dict] = None) -> str:
        ...
        """
        # BASELINE PROMPT - exact same format as mistral_narrative_generation.py (lines 117-122)
        # Mistral uses [INST]...[/INST] format, NOT Llama-style!
...
{input_note} [/INST] """
    ...
    def clean_narrative_output(self, text: str) -> str:
        """
        MINIMAL cleaning to EXACTLY match baseline behavior.

        Baseline (mistral_narrative_generation.py lines 157-164) ONLY does:
        1. Check if generation was cut off mid-sentence
        2. If so, keep only complete sentences

        NO aggressive section removal - that destroys valid content and kills ROUGE!
        """
        if not text:
            return text
        ...
        # Match baseline EXACTLY: Check if generation was cut off mid-sentence
        if text and text[-1] not in '.!?':
            # Find last complete sentence (matching baseline lines 157-164)
    ...
    def generate_narrative_summary(self, input_note: str, retrieved_cases: List[Dict] = None) -> str:
        ...
        # Build prompt (BASELINE - no example injection)
        ...
        # BASELINE LOGIC: Use dynamic max_tokens based on available space
        dynamic_max_tokens = min(NarrativeRAGConfig.MAX_NEW_TOKENS, max_possible_tokens - 10)

        # Generate with EXACT BASELINE parameters from mistral_narrative_generation.py
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                min_new_tokens=NarrativeRAGConfig.MIN_NEW_TOKENS,  # 240 - force ~180 words
                max_new_tokens=dynamic_max_tokens,  # DYNAMIC based on input length
                temperature=NarrativeRAGConfig.TEMPERATURE,       # 0.6
                top_p=NarrativeRAGConfig.TOP_P,                   # 0.9
                top_k=NarrativeRAGConfig.TOP_K,                   # 50
                repetition_penalty=NarrativeRAGConfig.REPETITION_PENALTY,  # 1.05
                no_repeat_ngram_size=NarrativeRAGConfig.NO_REPEAT_NGRAM_SIZE,  # 3
                length_penalty=NarrativeRAGConfig.LENGTH_PENALTY,  # 1.2 (encourage longer)
