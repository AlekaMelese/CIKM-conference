...
1. PEFT (Phase 1): Fine-tuned Phi-3-Medium-14B model provides clinical domain adaptation
...
class RAGConfig:
    ...
    # Phase 1 fine-tuned model (LoRA adapters to avoid CUDA memory errors)
    PHASE1_MODEL_DIR = FINETUNING_DIR / "outputs" / "final_model"

    # RAG-specific directories (using /scratch to avoid space issues)
    SCRATCH_DIR = Path("./Final/Phi/RAG1_scratch")
    RAG_DATA_DIR = BASE_DIR / "data"
    RAG_MODELS_DIR = BASE_DIR / "models"
    RAG_OUTPUTS_DIR = SCRATCH_DIR / "outputs2"  # Save to scratch directory
    RAG_LOGS_DIR = SCRATCH_DIR / "logs"
    ...
    # Phase 1 fine-tuned Phi-3-Medium model path
...
"""
    ...
    def print_config(cls):
        ...
        print(f"\n🤖 MODELS")
        print(f"  Generator (PEFT): Fine-tuned Phi-3-Medium-14B (Phase 1)")
