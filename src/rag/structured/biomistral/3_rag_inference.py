...
WORKFLOW:
1. Fine-tuned Mistral model (Phase 1) - Domain-adapted for clinical summaries
...
class HybridPEFTRAGInference:
    ...
    def __init__(self):
        ...
        print("Fine-tuned BioMistral-7B + Structure-Focused Retrieval")
        print("=" * 80)
    ...
    def _load_finetuned_model(self):
        """Load Phase 1 fine-tuned BioMistral model (Unsloth or regular transformers)"""
    ...
    def _load_retrieval_systems(self):
        ...
        self.embedder = SentenceTransformer(RAGConfig.EMBEDDING_MODEL_NAME)
        print(f"   ✓ Embedder loaded")

        # Load reranker
        print(f"   Loading reranker: {RAGConfig.RERANKER_MODEL_NAME}")
        self.reranker = CrossEncoder(RAGConfig.RERANKER_MODEL_NAME)
...
•  Follow-Up & Recommendations:
    ...
    def generate_structured_summary_constrained(self, input_text: str) -> str:
        ...
        UPDATED: Increased token limits by 3x to improve content coverage and ROUGE-1 scores
        Previous avg coverage: 31.5% of target length (2x increase)
        Target: Increase to ~60-70% coverage while maintaining structure quality
        Repetition penalty reduced to 1.15 to allow medical term repetition
        """

        sections = []

        # 1. Case Type (increased 50→150, 3x)
        case_type = self.generate_section(input_text, "case_type", max_tokens=150)
        sections.append(f"Case Type: {case_type}")

        # 2. Patient & Service (increased 30→90, 3x)
        patient_service = self.generate_section(input_text, "patient_service", max_tokens=90)
        sections.append(f"\n\n•  Patient & Service: {patient_service}")

        # 3. Chief Complaint (increased 100→300, 3x)
