...
class LlamaNarrativeConfig:
    """Configuration for Llama-3.1-8B fine-tuning on NARRATIVE summaries - 5000 samples"""
...
def create_narrative_prompt_with_examples(input_text: str, target_text: str = None,
    ...
    """
    Create Llama-3.1-Instruct format prompt for WELL-ORGANIZED NARRATIVE summary generation
...
def prepare_dataset(config: LlamaNarrativeConfig):
    ...
    print("\n✓ Creating Llama-3.1-format narrative prompts...")
    train_prompts = []
...
def load_model(config: LlamaNarrativeConfig):
...
def train_model(model, tokenizer, train_dataset, val_dataset, config: LlamaNarrativeConfig):
    """Train Llama-3.1 model for narrative generation"""
...
def save_training_logs(trainer, config: LlamaNarrativeConfig, time_callback=None):
...
def save_models(model, tokenizer, config: LlamaNarrativeConfig):
...
def save_metadata(config: LlamaNarrativeConfig, train_result, df: pd.DataFrame):
    ...
    metadata_path = f"{config.OUTPUT_DIR}/llama_narrative_metadata.json"
...
def main():
    ...
    print("\n" + "="*80)
    print(" "*15 + "LLAMA-3.1-8B-INSTRUCT NARRATIVE FINE-TUNING")
    print(" "*8 + "Well-Organized Medical Narrative Summaries - 5000 Samples")
    print("="*80)

    # Initialize config
    config = LlamaNarrativeConfig()
    ...
    create_narrative_training_plots(trainer, config, model_name="LLaMA-3.1-8B Narrative", df=df)
    ...
    print(f"   - Metadata: {config.OUTPUT_DIR}/llama_narrative_metadata.json")
