...
import sys
sys.path.append('./Final/Qwen/Finetuning')
...
class GemmaNarrativeGenerator:
    ...
    def __init__(self, model_path: str = "./Final/Qwen/Narrative/Finetuning/outputs/final_model"):
        """Load the narrative fine-tuned Qwen model"""

        print("="*80)
        print("QWEN2-7B NARRATIVE MEDICAL SUMMARY GENERATOR")
        ...
        # Set max_seq_length to accommodate both input + output
        self.max_seq_length = 4096
        self.max_input_length = 3072  # Reserve ~1024 tokens for generation
        ...
        if torch.cuda.is_available():
            ...
            print(f"✅ Narrative Qwen model loaded successfully")
            print(f"   GPU Memory: {allocated:.2f} GB allocated, {reserved:.2f} GB reserved")
            print(f"   Max input length: {self.max_input_length} tokens")
            print(f"   Max generation: ~{self.max_seq_length - self.max_input_length} tokens\n")
        else:
            print("✅ Narrative Qwen model loaded successfully")
            print(f"   Max input length: {self.max_input_length} tokens")
            print(f"   Max generation: ~{self.max_seq_length - self.max_input_length} tokens\n")
    ...
    def generate_narrative_summary(self, input_text: str, temperature: float = 0.6,
        ...
        # This matches the updated qwen_narrative_finetune.py format
...
"""
        ...
        # Truncate input to max_input_length to leave room for generation
        ...
        max_possible_tokens = self.max_seq_length - input_token_count - 50

        # Choose max_new_tokens conservatively
        if max_possible_tokens < 512:
            dynamic_max_tokens = max(256, max_possible_tokens)  # Very long input, shorter output
        elif input_token_count > 2500:  # Long input
            dynamic_max_tokens = min(768, max_possible_tokens)
        else:  # Normal input
...
def evaluate_test_set(num_samples: int = None):
    ...
    output_dir = "./Final/Qwen/Narrative/Finetuning/outputs"
    ...
    results_path = f'{output_dir}/qwen_narrative_results.json'
    with open(results_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"  ✓ Saved: qwen_narrative_results.json")
    ...
    # Save metrics
    metrics_path = f'{output_dir}/qwen_narrative_metrics.json'
    with open(metrics_path, 'w') as f:
        json.dump(test_metrics, f, indent=2)
    print(f"  ✓ Saved: qwen_narrative_metrics.json")
    ...
    word_export_path = os.path.join(output_dir, 'qwen_narrative_results_for_word.txt')
