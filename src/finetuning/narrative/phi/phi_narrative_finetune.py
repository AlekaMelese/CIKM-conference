...
class PhiNarrativeConfig:
    """Configuration for Phi-7B fine-tuning on NARRATIVE summaries - 5000 samples"""
    ...
    LORA_R = 128  # IMPROVED: Increased from 64
    LORA_ALPHA = 128  # IMPROVED: Matches rank
    LORA_DROPOUT = 0.15  # IMPROVED: Increased from 0.1
    ...
    LEARNING_RATE = 5e-5  # IMPROVED: Reduced from 1e-4
    NUM_EPOCHS = 3
    WARMUP_STEPS = 10
    MAX_GRAD_NORM = 1.0
    WEIGHT_DECAY = 0.05
    ...
    OUTPUT_DIR = "./Final/Phi/Narrative/Finetuning/outputs"
...
def create_narrative_prompt_with_examples(input_text: str, target_text: str = None,
                                           example_pool: list = None,
                                           max_input_chars: int = None) -> str:
    """
    Create Phi-Instruct format prompt for WELL-ORGANIZED NARRATIVE summary generation
    ...
    Args:
        max_input_chars: If provided, truncate input to fit within 4096 token limit
    """

    # Truncate input if needed to fit within 4096 token limit
    # This ensures ALL 4998 samples can be used for training
    if max_input_chars and len(input_text) > max_input_chars:
        input_text = input_text[:max_input_chars] + "..."

    # CLEAN SIMPLE PROMPT: NO examples, NO verbose instructions
    # Phi-3 CORRECT format: <|user|>...<|end|><|assistant|>...<|end|>
    # NOTE: Phi-3 does NOT support <|system|> tag - must include system instruction in user message
...
"""
    ...
    if target_text:
        ...
        # Phi-3 CORRECT format: <|user|>...<|end|><|assistant|>...<|end|>
        prompt = f"""<|user|>
You are a medical AI assistant. Here are two examples of narrative discharge summaries:

EXAMPLE 1:
Clinical Note: {examples[0]['input'][:1000]}...

Summary: {examples[0]['target']}
...
Summary: {examples[1]['target']}

---

Now write a well-organized narrative discharge summary for this clinical note. Use flowing paragraphs without bullet points or headers.
...
def prepare_dataset(config: PhiNarrativeConfig):
    ...
    # CRITICAL FIX: Truncate inputs to 4000 chars to ensure ALL prompts fit within 4096 tokens
    # With 4000 chars: only 0.8% samples exceed limit (40/4998)
    # With 8000 chars: 14.7% samples exceed limit (filtered by SFTTrainer)
    MAX_INPUT_CHARS = 4000  # Strict limit to ensure ALL samples used for training
    print(f"\n✓ Creating Phi-format narrative prompts (max input: {MAX_INPUT_CHARS} chars)...")
    train_prompts = []
    truncated_count = 0
    for idx, row in train_df.iterrows():
        ...
        prompt = create_narrative_prompt_with_examples(
            ...
            max_input_chars=MAX_INPUT_CHARS  # Truncate long inputs
        )
        train_prompts.append(prompt)
    print(f"  Truncated {truncated_count} long inputs ({100*truncated_count/len(train_df):.1f}%)")
    ...
    print(f"✓ Created narrative datasets:")
    print(f"  Train: {len(train_dataset)} prompts (expected: 4498)")
    print(f"  Val:   {len(val_dataset)} prompts (expected: 300)")
    print(f"  ℹ️  Each prompt contains 2 real narrative examples from training data")

    # CRITICAL VERIFICATION
    if len(train_dataset) < 4000:
        print(f"\n⚠️  WARNING: Train dataset only has {len(train_dataset)} samples!")
        print(f"   Expected ~4498 samples. Data may have been filtered or split incorrectly.")
    else:
        print(f"\n✅ Train dataset size looks correct ({len(train_dataset)} samples)")

    return train_dataset, val_dataset, test_df, df


def load_model(config: PhiNarrativeConfig):
    """Load Phi-7B model with LoRA"""
...
def compute_metrics_wrapper(tokenizer):
    ...
    def compute_metrics(eval_pred):
        ...
        for pred, label in zip(decoded_preds, decoded_labels):
            # Find the response section for Phi format
            if "[/INST]" in pred:
                pred = pred.split("[/INST]")[-1].strip()
            if "[/INST]" in label:
                label = label.split("[/INST]")[-1].strip()
...
def train_model(model, tokenizer, train_dataset, val_dataset, config: PhiNarrativeConfig):
    """Train Phi model for narrative generation"""
    ...
    print(f"  Expected steps per epoch: {len(train_dataset) // config.BATCH_SIZE}")
    print(f"  Expected total steps: {(len(train_dataset) // config.BATCH_SIZE) * config.NUM_EPOCHS}")
    ...
    # CRITICAL FIX: Use num_of_sequences parameter to ensure ALL samples are used
    trainer = SFTTrainer(
        model=model,
        tokenizer=tokenizer,
        ...
        max_seq_length=config.MAX_SEQ_LENGTH,
        packing=False,  # CRITICAL: Disable packing to train on ALL 4498 samples (not just ~2252)
        dataset_num_proc=1,  # Disable parallel processing to prevent sample loss
        args=training_args,
    ...
    # VERIFICATION: Print actual training dataset length after SFTTrainer processes it
    print(f"\n🔍 CRITICAL VERIFICATION:")
    print(f"   Input train_dataset length: {len(train_dataset)}")
    print(f"   Trainer train_dataset length: {len(trainer.train_dataset) if hasattr(trainer, 'train_dataset') and trainer.train_dataset else 'N/A'}")
    print(f"   Expected total steps: {len(train_dataset) // config.BATCH_SIZE * config.NUM_EPOCHS}")
...
def save_training_logs(trainer, config: PhiNarrativeConfig, time_callback=None):
...
def save_models(model, tokenizer, config: PhiNarrativeConfig):
    ...
    try:
        ...
        print(f"⚠️  Merged model save failed (known Phi issue): {e}")
        print("✓ LoRA adapters saved successfully - use these for inference")

    print("\n✓ Model saving complete!")


def save_metadata(config: PhiNarrativeConfig, train_result, df: pd.DataFrame):
    ...
    metadata_path = f"{config.OUTPUT_DIR}/phi_narrative_metadata.json"
...
def main():
    ...
    print("\n" + "="*80)
    print(" "*15 + "PHI-7B-INSTRUCT NARRATIVE FINE-TUNING")
    print(" "*8 + "Well-Organized Medical Narrative Summaries - 5000 Samples")
    print("="*80)

    # Initialize config
    config = PhiNarrativeConfig()
    ...
    create_narrative_training_plots(trainer, config, model_name="Phi-7B Narrative", df=df)
    ...
    print(f"   - Metadata: {config.OUTPUT_DIR}/phi_narrative_metadata.json")
