...
class MistralNarrativeConfig:
    """Configuration for Mistral-7B fine-tuning on NARRATIVE summaries - 5000 samples"""
...
def create_narrative_prompt_with_examples(input_text: str, target_text: str = None,
    ...
    """
    Create Mistral-Instruct format prompt for WELL-ORGANIZED NARRATIVE summary generation
...
def prepare_dataset(config: MistralNarrativeConfig):
    ...
    print("\n✓ Creating Mistral-format narrative prompts...")
    train_prompts = []
...
def load_model(config: MistralNarrativeConfig):
    """Load Mistral-7B model with LoRA"""
...
def train_model(model, tokenizer, train_dataset, val_dataset, config: MistralNarrativeConfig):
    """Train Mistral model for narrative generation"""
...
def save_training_logs(trainer, config: MistralNarrativeConfig, time_callback=None):
...
def save_models(model, tokenizer, config: MistralNarrativeConfig):
    ...
    try:
        ...
        print(f"⚠️  Merged model save failed (known Mistral issue): {e}")
        print("✓ LoRA adapters saved successfully - use these for inference")

    print("\n✓ Model saving complete!")


def save_metadata(config: MistralNarrativeConfig, train_result, df: pd.DataFrame):
    ...
    metadata_path = f"{config.OUTPUT_DIR}/mistral_narrative_metadata.json"
...
def main():
    ...
    print("\n" + "="*80)
    print(" "*15 + "MISTRAL-7B-INSTRUCT NARRATIVE FINE-TUNING")
    print(" "*8 + "Well-Organized Medical Narrative Summaries - 5000 Samples")
    print("="*80)

    # Initialize config
    config = MistralNarrativeConfig()
    ...
    create_narrative_training_plots(trainer, config, model_name="Mistral-7B Narrative", df=df)
    ...
    print(f"   - Metadata: {config.OUTPUT_DIR}/mistral_narrative_metadata.json")
