...
class MistralConstrainedGenerator:
    """Generate structured medical summaries section-by-section using Mistral"""

    def __init__(self, model_path: str = "./Final/Mistral/Finetuning/outputs/final_model"):
        """Load the fine-tuned Mistral model"""

        print("="*80)
        print("MISTRAL CONSTRAINED MEDICAL SUMMARY GENERATOR")
        print("="*80)
...
def create_evaluation_plots(test_metrics: Dict, output_dir: str):
    ...
    fig.suptitle('Mistral-7B-Instruct: Test Set Performance', fontsize=16, fontweight='bold')
...
def export_for_word_format(results: List[Dict], output_file: str, num_samples: int = None):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("="*100 + "\n")
        f.write("MISTRAL-7B-INSTRUCT TEST SET RESULTS\n")
...
def evaluate_test_set():
    """Evaluate fine-tuned Mistral on test set ONLY"""

    print("="*80)
    print("MISTRAL TEST SET EVALUATION")
    print("="*80)

    output_dir = "./Final/Mistral/Finetuning/outputs"

    # Load generator
    generator = MistralConstrainedGenerator()
    ...
    results_path = f'{output_dir}/mistral_test_results.json'
    with open(results_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"  ✓ Saved: mistral_test_results.json")
    ...
    # Save metrics
    metrics_path = f'{output_dir}/mistral_test_metrics.json'
    with open(metrics_path, 'w') as f:
        json.dump({k: v for k, v in test_metrics.items() if not isinstance(v, list)}, f, indent=2)
    print(f"  ✓ Saved: mistral_test_metrics.json")
    ...
    word_export_path = os.path.join(output_dir, 'mistral_results_for_word.txt')
