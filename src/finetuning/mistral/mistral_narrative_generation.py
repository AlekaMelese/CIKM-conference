...
# Set cache directories to scratch (not home directory which is full)
...
import sys
sys.path.append('./Final/Mistral/Narrative/Finetuning')
from mistral_metrics import MedicalMetricsCalculator
...
class MistralNarrativeGenerator:
    """Generate NARRATIVE medical summaries using fine-tuned Mistral"""

    def __init__(self, model_path: str = "./Final/Mistral/Narrative/Finetuning/outputs/final_model"):
        """Load the narrative fine-tuned Mistral model"""

        print("="*80)
        print("MISTRAL-7B NARRATIVE MEDICAL SUMMARY GENERATOR")
        ...
        self.model, self.tokenizer = FastLanguageModel.from_pretrained(
            ...
            max_seq_length=4096,  # Matches training configuration
            dtype=None,
            load_in_4bit=True,
        ...
        if torch.cuda.is_available():
            ...
            print(f"✅ Narrative Mistral model loaded successfully")
            print(f"   GPU Memory: {allocated:.2f} GB allocated, {reserved:.2f} GB reserved\n")
        else:
            print("✅ Narrative Mistral model loaded successfully\n")
    ...
    def generate_narrative_summary(self, input_text: str, temperature: float = 0.6,
        ...
        # This matches the Mistral-Instruct format that was used during training
...
{input_text} [/INST] """
        ...
        # Dynamic max_tokens based on available space
        input_token_count = inputs['input_ids'].shape[1]
        max_seq_length = 4096  # Matches model loading
        available_tokens = max_seq_length - input_token_count
        dynamic_max_tokens = min(max_tokens, available_tokens - 10)  # Leave small buffer
        ...
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                ...
                top_p=0.9,
                top_k=50,
                repetition_penalty=1.05,  # Reduced from 1.1 - too strong causes garbage output
    ...
    def _clean_narrative_output(self, text: str) -> str:
        """Remove any unwanted formatting that may have slipped through - MINIMAL CLEANING"""
        import re

        # If the output is just echoing the prompt instructions, return empty
        # Check for common prompt instruction phrases that indicate prompt echo
        prompt_echo_phrases = [
            "You are a medical AI assistant",
            "Write a well-organized narrative discharge summary from the clinical note below",
            "Use flowing paragraphs without bullet points or section headers"
        ]
        if any(phrase in text for phrase in prompt_echo_phrases):
            # This is prompt echo, not actual generation
            return ""
        ...
        # If output is empty or too short (< 50 words), likely failed generation
        word_count = len(text.split())
        if word_count < 50:
            return ""  # Narrative summaries should be at least 50 words
...
def export_for_word_format(results: List[Dict], output_file: str, num_samples: int = None):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("="*100 + "\n")
        f.write("MISTRAL-7B-INSTRUCT NARRATIVE TEST SET RESULTS\n")
...
def display_comprehensive_metrics(metrics: Dict):
    ...
    print(" "*20 + "Mistral-7B Narrative Model")
    print("="*80)
...
def evaluate_test_set(num_samples: int = None):
    """
    Evaluate narrative fine-tuned Mistral on test set
    ...
    print("="*80)
    print("MISTRAL NARRATIVE TEST SET EVALUATION")
    print("="*80)

    output_dir = "./Final/Mistral/Narrative/Finetuning/outputs"

    # Load generator
    generator = MistralNarrativeGenerator()
    ...
    results_path = f'{output_dir}/mistral_narrative_results.json'
    with open(results_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"  ✓ Saved: mistral_narrative_results.json")
    ...
    # Save metrics
    metrics_path = f'{output_dir}/mistral_narrative_metrics.json'
    with open(metrics_path, 'w') as f:
        json.dump(test_metrics, f, indent=2)
    print(f"  ✓ Saved: mistral_narrative_metrics.json")
    ...
    word_export_path = os.path.join(output_dir, 'mistral_narrative_results_for_word.txt')
