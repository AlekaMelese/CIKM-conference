#!/usr/bin/env python3
"""
Phi-3-Medium-4k-Instruct: NARRATIVE Generation Evaluation on Test Set

Evaluates the NARRATIVE fine-tuned Phi model on the test set (200 samples).
...
import sys
sys.path.append('./Final/Phi/Narrative/Finetuning')
...
class PhiNarrativeGenerator:
    """Generate NARRATIVE medical summaries using fine-tuned Phi"""

    def __init__(self, model_path: str = "./Final/Phi/Narrative/Finetuning/outputs/final_model"):
        """Load the narrative fine-tuned Phi model"""

        print("="*80)
        print("PHI-7B NARRATIVE MEDICAL SUMMARY GENERATOR")
        ...
        if torch.cuda.is_available():
            ...
            print(f"✅ Narrative Phi model loaded successfully")
            print(f"   GPU Memory: {allocated:.2f} GB allocated, {reserved:.2f} GB reserved\n")
        else:
            print("✅ Narrative Phi model loaded successfully\n")

    def generate_narrative_summary(self, input_text: str, temperature: float = 0.6,
                                   max_tokens: int = 512, min_words: int = 150) -> str:
        """
        Generate comprehensive narrative paragraph summary

        Parameters:
        - temperature: 0.6 (balanced creativity for narrative flow)
        - max_tokens: 512 (reduced from 768 to prevent over-generation ~400 words)
        - min_words: 150 (minimum target word count for comprehensive summaries)
        """

        # Prompt MUST match training format exactly (CLEAN and SIMPLE)
        # Phi-3 CORRECT format: <|user|>...<|end|><|assistant|>
...
"""
        ...
        # Dynamic max_tokens based on input length to avoid exceeding max_seq_length
        ...
        if max_possible_tokens < 512:
            dynamic_max_tokens = min(200, max_possible_tokens - 10)  # Very long input, shorter output but safe
        elif input_token_count > 2500:  # Long input
            dynamic_max_tokens = min(512, max_possible_tokens)
        else:  # Normal input
            dynamic_max_tokens = min(max_tokens, max_possible_tokens)
        ...
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                ...
                no_repeat_ngram_size=0,  # Disabled - can cause issues with medical terminology
...
def export_for_word_format(results: List[Dict], output_file: str, num_samples: int = None):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("="*100 + "\n")
        f.write("PHI-7B-INSTRUCT NARRATIVE TEST SET RESULTS\n")
...
def display_comprehensive_metrics(metrics: Dict):
    ...
    print(" "*20 + "Phi-7B Narrative Model")
    print("="*80)
...
def evaluate_test_set(num_samples: int = None):
    """
    Evaluate narrative fine-tuned Phi on test set
    ...
    print("="*80)
    print("PHI NARRATIVE TEST SET EVALUATION")
    print("="*80)

    output_dir = "./Final/Phi/Narrative/Finetuning/outputs"

    # Load generator
    generator = PhiNarrativeGenerator()
    ...
    results_path = f'{output_dir}/phi_narrative_results.json'
    with open(results_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"  ✓ Saved: phi_narrative_results.json")
    ...
    # Save metrics
    metrics_path = f'{output_dir}/phi_narrative_metrics.json'
    with open(metrics_path, 'w') as f:
        json.dump(test_metrics, f, indent=2)
    print(f"  ✓ Saved: phi_narrative_metrics.json")
    ...
    word_export_path = os.path.join(output_dir, 'phi_narrative_results_for_word.txt')
