#!/usr/bin/env python3
"""
Phi-3-Medium-4K-Instruct Fine-tuning for Medical Discharge Summary Generation
Extended Training on 5000 Samples

Fine-tunes Phi-3-Medium-4K-Instruct (14B, strong reasoning) on structured medical summaries using:
...
class PhiConfig:
    """Configuration for Phi-3-Medium-4K-Instruct (14B) fine-tuning on 5000 samples

    MEMORY-OPTIMIZED FOR A30 24GB:
    - Phi-3-Medium (14B parameters) with strong reasoning capabilities
    - BATCH_SIZE = 1 (reduced from 4) to fit in A30 24GB
    - GRADIENT_ACCUMULATION_STEPS = 8 (increased from 2) to maintain effective batch size of 8
    - 4-bit quantization for memory efficiency
    """

    # Model settings
    MODEL_NAME = "unsloth/Phi-3-medium-4k-instruct-bnb-4bit"  # 14B parameters
    MAX_SEQ_LENGTH = 4096
    DTYPE = None  # Auto-detect
    LOAD_IN_4BIT = True

    # LoRA settings (optimized for Phi-3-Medium)
    ...
    # Training settings (MEMORY-OPTIMIZED for A30 24GB)
    BATCH_SIZE = 1  # Reduced from 4 to fit 14B model in 24GB GPU
    GRADIENT_ACCUMULATION_STEPS = 8  # Increased from 2, effective batch size = 1 * 8 = 8
...
def create_phi_prompt_with_examples(input_text: str, target_text: str = None,
                                     example_pool: list = None) -> str:
    """Create Phi-3-Instruct format prompt with 2 few-shot examples from actual training data"""
...
You are a medical AI assistant specialized in generating structured discharge summaries.<|eot_id|><|start_header_id|>user<|end_header_id|>
...
- Discharge Condition
- Follow-Up & Recommendations<|eot_id|><|start_header_id|>assistant<|end_header_id|>
...
def create_phi_prompt(input_text: str, target_text: str = None) -> str:
    """Original create prompt function - kept for backward compatibility"""
    return create_phi_prompt_with_examples(input_text, target_text, example_pool=None)


def prepare_dataset(config: PhiConfig):
    ...
    print("\n✓ Creating Phi-3-format prompts with few-shot examples...")
    train_prompts = []
    for idx, row in train_df.iterrows():
        ...
        prompt = create_phi_prompt_with_examples(
            row['input'],
...
def load_model(config: PhiConfig):
    """Load Phi-3-Medium-4K-Instruct model with LoRA

    For 14B model on A30 24GB: Uses CPU offloading with custom max_memory
    to explicitly allocate GPU memory and allow overflow to CPU/disk.
    """
    ...
    print("   ⚠️  14B model on A30 24GB requires CPU offloading")

    # Import required for quantization config
    ...
    bnb_config = BitsAndBytesConfig(
        ...
        llm_int8_enable_fp32_cpu_offload=True,  # Enable CPU offload for 14B model
    )

    # Set max memory: use 21GB of GPU, rest can overflow to CPU
    max_memory = {0: "21GB", "cpu": "100GB"}

    print(f"   Max GPU memory: 21GB (out of 24GB)")
    print(f"   CPU offload enabled for overflow")

    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name=config.MODEL_NAME,
        max_seq_length=config.MAX_SEQ_LENGTH,
        dtype=config.DTYPE,
        load_in_4bit=False,  # Using bnb_config instead
...
def compute_metrics_wrapper(tokenizer):
    ...
    def compute_metrics(eval_pred):
        ...
        for pred, label in zip(decoded_preds, decoded_labels):
            # Find the response section for Llama-3.1 format
...
def train_model(model, tokenizer, train_dataset, val_dataset, config: PhiConfig):
    """Train Phi-3-Medium model"""
...
def save_training_logs(trainer, config: PhiConfig, time_callback=None):
...
def save_models(model, tokenizer, config: PhiConfig):
...
def create_publication_plots(trainer, train_result, config: PhiConfig, df: pd.DataFrame, time_callback=None):
    ...
    ax.set_title('Training & Validation Loss: Overfitting Analysis (5000 Samples) - Phi-3-Medium', fontsize=15, fontweight='bold')
    ...
    fig.suptitle('Comprehensive Training Metrics Dashboard (5000 Samples) - Phi-3-Medium-4K', fontsize=18, fontweight='bold')
...
def save_metadata(config: PhiConfig, train_result, df: pd.DataFrame):
    ...
    metadata_path = f"{config.OUTPUT_DIR}/phi_metadata.json"
...
def main():
    ...
    print("\n" + "="*80)
    print(" "*12 + "PHI-3-MEDIUM-4K-INSTRUCT FINE-TUNING")
    ...
    print(f"   - Metadata: {config.OUTPUT_DIR}/phi_metadata.json")
