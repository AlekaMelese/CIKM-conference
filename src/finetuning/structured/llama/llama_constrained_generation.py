...
class LlamaConstrainedGenerator:
    """Generate structured medical summaries section-by-section using Llama"""

    def __init__(self, model_path: str = "./Final/Llama/Finetuning/outputs/merged_model"):
        """Load the fine-tuned Llama model"""

        print("="*80)
        print("LLAMA CONSTRAINED MEDICAL SUMMARY GENERATOR")
        print("="*80)
        ...
        print("✅ Llama fine-tuned model loaded successfully\n")
    ...
    def generate_section(self, input_text: str, section_name: str,
        ...
        """Generate a single section with focused Llama-format prompt
...
def create_evaluation_plots(test_metrics: Dict, output_dir: str):
    ...
    fig.suptitle('Llama-3.1-8B-Instruct: Test Set Performance', fontsize=16, fontweight='bold')
...
def evaluate_test_set(num_samples: int = None):
    """Evaluate fine-tuned Llama on test set ONLY
    ...
    print("="*80)
    print("LLAMA TEST SET EVALUATION")
    print("="*80)

    output_dir = "./Final/Llama/Finetuning/outputs"

    # Load generator
    generator = LlamaConstrainedGenerator()
    ...
    results_path = f'{output_dir}/llama_test_results.json'
    with open(results_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"  ✓ Saved: llama_test_results.json")
    ...
    # Save metrics
    metrics_path = f'{output_dir}/llama_test_metrics.json'
    with open(metrics_path, 'w') as f:
        json.dump({k: v for k, v in test_metrics.items() if not isinstance(v, list)}, f, indent=2)
    print(f"  ✓ Saved: llama_test_metrics.json")
    ...
    word_export_path = os.path.join(output_dir, 'llama_results_for_word.txt')
