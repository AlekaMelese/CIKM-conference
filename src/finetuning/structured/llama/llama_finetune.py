...
class LlamaConfig:
    """Configuration for Llama-3.1-8B fine-tuning on 5000 samples"""
    ...
    # LoRA settings (optimized for Llama 3.1)
...
def create_llama_prompt_with_examples(input_text: str, target_text: str = None,
                                       example_pool: list = None) -> str:
    """Create Llama-3.1-Instruct format prompt with 2 few-shot examples from actual training data"""
...
def create_llama_prompt(input_text: str, target_text: str = None) -> str:
    """Original create prompt function - kept for backward compatibility"""
    return create_llama_prompt_with_examples(input_text, target_text, example_pool=None)


def prepare_dataset(config: LlamaConfig):
    ...
    print("\n✓ Creating Llama-3.1-format prompts with few-shot examples...")
    train_prompts = []
    for idx, row in train_df.iterrows():
        ...
        prompt = create_llama_prompt_with_examples(
            row['input'],
...
def load_model(config: LlamaConfig):
    """Load Llama-3.1-8B model with LoRA"""
...
def train_model(model, tokenizer, train_dataset, val_dataset, config: LlamaConfig):
    """Train Llama-3.1 model"""
...
def save_training_logs(trainer, config: LlamaConfig, time_callback=None):
...
def save_models(model, tokenizer, config: LlamaConfig):
...
def create_publication_plots(trainer, train_result, config: LlamaConfig, df: pd.DataFrame, time_callback=None):
    ...
    fig.suptitle('Comprehensive Training Metrics Dashboard (5000 Samples) - Llama 3.1-8B', fontsize=18, fontweight='bold')
...
def save_metadata(config: LlamaConfig, train_result, df: pd.DataFrame):
    ...
    metadata_path = f"{config.OUTPUT_DIR}/llama_metadata.json"
...
def main():
    ...
    print(f"   - Metadata: {config.OUTPUT_DIR}/llama_metadata.json")
