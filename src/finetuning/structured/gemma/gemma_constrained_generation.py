...
from medical_metrics import MedicalMetricsCalculator
...
class GemmaConstrainedGenerator:
    """Generate structured medical summaries section-by-section using Gemma"""

    def __init__(self, model_path: str = "./Final/Gemma/Finetuning/outputs/final_model"):
        """Load the fine-tuned Gemma model"""

        print("="*80)
        print("GEMMA CONSTRAINED MEDICAL SUMMARY GENERATOR")
        print("="*80)
...
Generate only the case type line. [/INST]

Case Type:""",

            "patient_service": f"""[INST] Based on this clinical note, write ONLY the patient gender and medical service:

Clinical Note: {input_text[:1000]}

Generate only the patient and service line. [/INST]

•  Patient & Service:""",

            "chief_complaint": f"""[INST] Based on this clinical note, write ONLY the chief complaint and admission context:

Clinical Note: {input_text[:1500]}

Generate only the chief complaint. [/INST]

•  Chief Complaint / Admission Context:""",

            "hpi": f"""[INST] Based on this clinical note, write ONLY the history of present illness (2-3 sentences):

Clinical Note: {input_text[:2000]}

Generate only the HPI. [/INST]

•  History of Present Illness (HPI):""",

            "pmh": f"""[INST] Based on this clinical note, write ONLY the past medical/surgical history (comma-separated list):

Clinical Note: {input_text[:1500]}

Generate only the medical history. [/INST]

•  Past Medical / Surgical History:""",

            "medications": f"""[INST] Based on this clinical note, write the discharge medications:

Clinical Note: {input_text[:2000]}

Generate only the discharge medications. [/INST]

•  Medications (Discharge / Ongoing):
    •  Discharge:""",

            "physical_exam": f"""[INST] Based on this clinical note, write ONLY the key physical examination findings:

Clinical Note: {input_text[:1800]}

Generate only the physical exam findings. [/INST]

•  Physical Examination (summarized):""",

            "labs": f"""[INST] Based on this clinical note, write ONLY the key lab results and imaging findings:

Clinical Note: {input_text[:1800]}

Generate only the lab/imaging results. [/INST]

•  Investigations / Labs / Imaging (if any):""",

            "assessment": f"""[INST] Based on this clinical note, write ONLY the final assessment/diagnosis:

Clinical Note: {input_text[:1500]}

Generate only the assessment. [/INST]

•  Assessment / Impression:""",

            "discharge_condition": f"""[INST] Based on this clinical note, write ONLY the patient's discharge condition:

Clinical Note: {input_text[:1500]}

Generate only the discharge condition. [/INST]

•  Discharge Condition:""",

            "followup": f"""[INST] Based on this clinical note, write the follow-up medication changes:

Clinical Note: {input_text[:2000]}

Generate only the medication changes. [/INST]
...
•  Follow-Up & Recommendations:
        ...
        with torch.no_grad():
            outputs = self.model.generate(
                **inputs,
                ...
                do_sample=True,
                top_p=0.95,                  # 0.95 (was 0.9 - allow more vocab diversity)
                repetition_penalty=1.1,      # 1.1 (was 1.3 - KEY CHANGE - allow more lexical overlap)
    ...
    def generate_structured_summary(self, input_text: str) -> str:
        ...
        hpi = self.generate_section(input_text, "hpi", max_tokens=300)  # Increased from 200
        ...
        pe = self.generate_section(input_text, "physical_exam", max_tokens=250)  # Increased from 150
        sections.append(f"\n\n•  Physical Examination (summarized): {pe}")

        # 8. Labs/Imaging
        labs = self.generate_section(input_text, "labs", max_tokens=250)  # Increased from 200
        sections.append(f"\n\n•  Investigations / Labs / Imaging (if any): {labs}")

        # 9. Assessment/Impression
        assessment = self.generate_section(input_text, "assessment", max_tokens=150)  # Increased from 100
...
def create_evaluation_plots(test_metrics: Dict, output_dir: str):
    ...
    fig.suptitle('Gemma-2-9B: Test Set Performance', fontsize=16, fontweight='bold')
...
def export_for_word_format(results: List[Dict], output_file: str, num_samples: int = None):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("="*100 + "\n")
        f.write("GEMMA-2-9B TEST SET RESULTS\n")
        ...
        for idx, sample in enumerate(results):
            ...
            if 'medical' in metrics:
                ...
                f.write(f"  BLEU Score:                  {med.get('bleu_score', 0):.4f}\n\n")

                f.write("Hallucination Analysis:\n")
                f.write(f"  Hallucination Coverage:      {med.get('hallucination_coverage', 0):.4f} ({med.get('hallucination_coverage', 0)*100:.2f}%)\n")
                f.write(f"  Hallucination Rate:          {med.get('hallucination_rate', 0):.4f} ({med.get('hallucination_rate', 0)*100:.2f}%)\n")
                f.write(f"  Grounded Sentences:          {med.get('grounded_sentences', 0)}\n")
                f.write(f"  Total Sentences:             {med.get('total_sentences', 0)}\n\n")
...
def evaluate_test_set(num_samples=None):
    """Evaluate fine-tuned Gemma on test set ONLY"""

    print("="*80)
    print("GEMMA TEST SET EVALUATION")
    print("="*80)

    output_dir = "./Final/Gemma/Finetuning/outputs"

    # Load generator
    generator = GemmaConstrainedGenerator()
    ...
    if num_samples:
        ...
        print(f"\n📊 Test set loaded: {num_samples} samples (LIMITED FOR TESTING)")
    else:
    ...
    results_path = f'{output_dir}/gemma_test_results.json'
    with open(results_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"  ✓ Saved: gemma_test_results.json")
    ...
    test_metrics = {
        ...
        # Hallucination metrics (mean and std)
        'hallucination_coverage': np.mean([r['metrics']['medical'].get('hallucination_coverage', 0) for r in results]),
        'hallucination_coverage_std': np.std([r['metrics']['medical'].get('hallucination_coverage', 0) for r in results]),
        'hallucination_rate': np.mean([r['metrics']['medical'].get('hallucination_rate', 0) for r in results]),
        'hallucination_rate_std': np.std([r['metrics']['medical'].get('hallucination_rate', 0) for r in results]),

        # Readability metrics (mean and std)
        'flesch_reading_ease': np.mean([r['metrics']['medical'].get('flesch_reading_ease', 0) for r in results]),
        'flesch_reading_ease_std': np.std([r['metrics']['medical'].get('flesch_reading_ease', 0) for r in results]),
        'flesch_kincaid_grade': np.mean([r['metrics']['medical'].get('flesch_kincaid_grade', 0) for r in results]),
        'flesch_kincaid_grade_std': np.std([r['metrics']['medical'].get('flesch_kincaid_grade', 0) for r in results]),

        # BLEU metric (mean and std)
        'bleu': np.mean([r['metrics']['medical'].get('bleu_score', 0) for r in results]),
        'bleu_std': np.std([r['metrics']['medical'].get('bleu_score', 0) for r in results]),
    ...
    # Save metrics
    metrics_path = f'{output_dir}/gemma_test_metrics.json'
    with open(metrics_path, 'w') as f:
        json.dump({k: v for k, v in test_metrics.items() if not isinstance(v, list)}, f, indent=2)
    print(f"  ✓ Saved: gemma_test_metrics.json")
    ...
    print(f"    Hallucination Coverage: {test_metrics['hallucination_coverage']:.4f} ± {test_metrics['hallucination_coverage_std']:.4f} ({test_metrics['hallucination_coverage']*100:.2f}%)")
    print(f"    Hallucination Rate: {test_metrics['hallucination_rate']:.4f} ± {test_metrics['hallucination_rate_std']:.4f} ({test_metrics['hallucination_rate']*100:.2f}%)")

    print(f"\n  GENERATION QUALITY:")
    print(f"    BLEU: {test_metrics['bleu']:.4f} ± {test_metrics['bleu_std']:.4f}")

    print(f"\n  READABILITY:")
    print(f"    Flesch Reading Ease: {test_metrics['flesch_reading_ease']:.2f} ± {test_metrics['flesch_reading_ease_std']:.2f}")
    print(f"    Flesch-Kincaid Grade: {test_metrics['flesch_kincaid_grade']:.2f} ± {test_metrics['flesch_kincaid_grade_std']:.2f} (College level)")
    ...
    word_export_path = os.path.join(output_dir, 'gemma_results_for_word.txt')
