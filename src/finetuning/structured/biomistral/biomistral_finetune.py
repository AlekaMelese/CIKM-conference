#!/usr/bin/env python3
"""
BioMistral-7B Fine-tuning for Medical Discharge Summary Generation
Extended Training on 5000 Samples

Fine-tunes BioMistral-7B (medical domain pre-trained model) on structured medical summaries using:
...
BioMistral-7B is pre-trained on PubMed Central biomedical literature,
providing stronger medical terminology understanding than general models.
"""
...
class BioMistralConfig:
    """Configuration for BioMistral-7B fine-tuning on 5000 samples"""

    # Model settings - BioMistral-7B pre-trained on medical data
...
def create_biomistral_prompt_with_examples(input_text: str, target_text: str = None,
                                           example_pool: list = None) -> str:
    """Create BioMistral format prompt with 2 few-shot examples from actual training data

    BioMistral-7B uses the same Mistral-Instruct format but benefits from medical pre-training.
    """

    # If no example pool provided, fall back to basic prompt
    if example_pool is None or len(example_pool) < 2:
        if target_text:
            prompt = f"""[INST] You are a medical AI assistant. Generate a structured discharge summary from the clinical note below.
...
def create_biomistral_prompt(input_text: str, target_text: str = None) -> str:
    """Original create prompt function - kept for backward compatibility"""
    return create_biomistral_prompt_with_examples(input_text, target_text, example_pool=None)


def prepare_dataset(config: BioMistralConfig):
    ...
    print("\n✓ Creating BioMistral-format prompts with few-shot examples...")
    train_prompts = []
    for idx, row in train_df.iterrows():
        ...
        prompt = create_biomistral_prompt_with_examples(
            row['input'],
...
def load_model(config: BioMistralConfig):
    """Load BioMistral-7B model with LoRA"""

    print("\n"+"="*80)
    print("MODEL LOADING - BioMistral-7B")
    print("="*80)

    print(f"\n📥 Loading BioMistral-7B base model: {config.MODEL_NAME}")
    print("   (Pre-trained on PubMed Central biomedical literature)")
    ...
    print("✓ BioMistral-7B base model loaded")
...
def compute_metrics_wrapper(tokenizer):
    ...
    def compute_metrics(eval_pred):
        ...
        # If predictions are logits (3D: batch, seq_len, vocab_size), take argmax
        ...
        for pred, label in zip(decoded_preds, decoded_labels):
            # Find the response section for Mistral format
            if "[/INST]" in pred:
...
def train_model(model, tokenizer, train_dataset, val_dataset, config: BioMistralConfig):
    """Train BioMistral-7B model"""
...
def save_training_logs(trainer, config: BioMistralConfig, time_callback=None):
...
def save_models(model, tokenizer, config: BioMistralConfig):
    """Save fine-tuned BioMistral models"""
...
def create_publication_plots(trainer, train_result, config: BioMistralConfig, df: pd.DataFrame, time_callback=None):
    ...
    fig.suptitle('Comprehensive Training Metrics Dashboard (5000 Samples)', fontsize=18, fontweight='bold')
    ...
    fig.suptitle('Dataset Statistics (5000 Samples)', fontsize=16, fontweight='bold')
    ...
    plt.close()
    print("    ✓ Saved: 6_dataset_statistics.png/.pdf")

    # PLOT 7: Training Summary Dashboard with Training Time
    print("\n  Creating Plot 7: Training Summary Dashboard...")
    fig = plt.figure(figsize=(16, 10))
    fig.suptitle('BioMistral-7B: Comprehensive Training Summary (5000 Samples)', fontsize=18, fontweight='bold')
    ...
    plt.close()
    print("    ✓ Saved: 7_training_summary.png/.pdf")
...
def save_metadata(config: BioMistralConfig, train_result, df: pd.DataFrame):
    ...
    metadata = {
        "model_info": {
            ...
            "model_type": "BioMistral-7B (PubMed Central pre-trained)",
    ...
    metadata_path = f"{config.OUTPUT_DIR}/biomistral_metadata.json"
    ...
    print("  - BioMistral models ready for RAG integration")
    print("  - Configuration saved for explainability analysis")


def main():
    """Main training pipeline for BioMistral-7B"""

    print("\n" + "="*80)
    print(" "*15 + "BioMistral-7B FINE-TUNING")
    print(" "*10 + "Medical Discharge Summary Generation - 5000 Samples")
    print(" "*8 + "Pre-trained on PubMed Central Biomedical Literature")
    print("="*80)

    # Initialize config
    config = BioMistralConfig()
    ...
    print("\n" + "="*80)
    print("BioMistral-7B FINE-TUNING COMPLETE!")
    print("="*80)
    ...
    print(f"   - Metadata: {config.OUTPUT_DIR}/biomistral_metadata.json")

    print("\n📊 Next Steps:")
    print("   1. Run constrained generation evaluation on 200 test samples")
    print("   2. Compare BioMistral vs Mistral-7B-Instruct performance")
    print("   3. Phase 2: RAG integration with fine-tuned BioMistral model")
    print("   4. Phase 2: Explainability analysis")
