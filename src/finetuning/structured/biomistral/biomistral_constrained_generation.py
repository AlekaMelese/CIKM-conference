#!/usr/bin/env python3
"""
BioMistral-7B: Constrained Generation Evaluation on Test Set

Evaluates the fine-tuned BioMistral model ONLY on the test set (10% unseen data).
Includes:
...
class BioMistralConstrainedGenerator:
    """Generate structured medical summaries section-by-section using BioMistral"""

    def __init__(self, model_path: str = "./Final/BioMistral/Finetuning/outputs/final_model"):
        """Load the fine-tuned BioMistral model"""

        print("="*80)
        print("BIOMISTRAL CONSTRAINED MEDICAL SUMMARY GENERATOR")
        print("="*80)
    ...
    def generate_section(self, input_text: str, section_name: str,
        ...
        """Generate a single section with focused BioMistral-format prompt"""
...
•  Follow-Up & Recommendations:
    ...
    def generate_structured_summary(self, input_text: str) -> str:
        ...
        hpi = self.generate_section(input_text, "hpi", max_tokens=200)
        ...
        pe = self.generate_section(input_text, "physical_exam", max_tokens=150)
        sections.append(f"\n\n•  Physical Examination (summarized): {pe}")

        # 8. Labs/Imaging
        labs = self.generate_section(input_text, "labs", max_tokens=200)
        sections.append(f"\n\n•  Investigations / Labs / Imaging (if any): {labs}")

        # 9. Assessment/Impression
        assessment = self.generate_section(input_text, "assessment", max_tokens=100)
...
def calculate_bertscore_batch(generated_list: List[str], target_list: List[str]) -> Dict[str, List[float]]:
    ...
    if not METRICS_AVAILABLE or len(generated_list) == 0:
        print("      ⚠️  BERTScore skipped: metrics not available or empty list")
        return {
    ...
    try:
        print(f"      Computing BERTScore for {len(generated_list)} samples - this may take a few minutes...")
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        print(f"      Using device: {device}")

        P, R, F1 = bert_score(generated_list, target_list, lang='en', verbose=False, device=device)
        ...
        # Print summary stats
        print(f"      ✓ BERTScore computed successfully!")
        print(f"        Precision: mean={sum(precision_list)/len(precision_list):.4f}, range=[{min(precision_list):.4f}, {max(precision_list):.4f}]")
        print(f"        Recall:    mean={sum(recall_list)/len(recall_list):.4f}, range=[{min(recall_list):.4f}, {max(recall_list):.4f}]")
        print(f"        F1:        mean={sum(f1_list)/len(f1_list):.4f}, range=[{min(f1_list):.4f}, {max(f1_list):.4f}]")
        ...
        print(f"      ⚠️  BERTScore failed with error: {e}")
        import traceback
        traceback.print_exc()
        return {
            'precision': [0.0] * len(generated_list),
...
def create_evaluation_plots(test_metrics: Dict, output_dir: str):
    ...
    fig.suptitle('BioMistral-7B: Test Set Performance', fontsize=16, fontweight='bold')
...
def export_for_word_format(results: List[Dict], output_file: str, num_samples: int = None):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("="*100 + "\n")
        f.write("BIOMISTRAL-7B TEST SET RESULTS\n")
...
def evaluate_test_set(num_samples: int = None):
    """Evaluate fine-tuned BioMistral on test set ONLY
    ...
    print("="*80)
    print("BIOMISTRAL TEST SET EVALUATION")
    print("="*80)

    output_dir = "./Final/BioMistral/Finetuning/outputs"

    # Load generator
    generator = BioMistralConstrainedGenerator()
    ...
    if num_samples is not None:
        ...
        print(f"\n📊 Test set loaded: {len(test_df)} samples (LIMITED TO FIRST {num_samples})")
    else:
    ...
    results_path = f'{output_dir}/biomistral_test_results.json'
    with open(results_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"  ✓ Saved: biomistral_test_results.json")
    ...
    # Add BERTScore back to individual results
    ...
    # Re-save results with BERTScore included
    with open(results_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"  ✓ Updated results with BERTScore: biomistral_test_results.json")
    ...
    # Save metrics
    metrics_path = f'{output_dir}/biomistral_test_metrics.json'
    with open(metrics_path, 'w') as f:
        json.dump({k: v for k, v in test_metrics.items() if not isinstance(v, list)}, f, indent=2)
    print(f"  ✓ Saved: biomistral_test_metrics.json")
    ...
    word_export_path = os.path.join(output_dir, 'biomistral_results_for_word.txt')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print(f"\n🔢 Running on first {num_samples} samples (command-line argument)\n")
        except ValueError:
            print(f"\n⚠️  Invalid argument '{sys.argv[1]}' - must be an integer")
            print("Usage: python biomistral_constrained_generation.py [num_samples]")
            print("Example: python biomistral_constrained_generation.py 4")
            sys.exit(1)
