#!/usr/bin/env python3
"""
BioMistral-7B: NARRATIVE Generation Evaluation on Test Set

Evaluates the NARRATIVE fine-tuned BioMistral-7B model on the test set (200 samples).
...
import sys
sys.path.append('./Final/BioMistral/Finetuning')
from biomistral_metrics import MedicalMetricsCalculator
...
class BioMistralNarrativeGenerator:
    """Generate NARRATIVE medical summaries using fine-tuned BioMistral-7B"""

    def __init__(self):
        """Load the narrative fine-tuned BioMistral-7B model using base + LoRA"""

        print("="*80)
        print("BIOMISTRAL-7B NARRATIVE MEDICAL SUMMARY GENERATOR")
        ...
        if torch.cuda.is_available():
            ...
            print(f"   GPU Memory: {total_mem:.2f} GB total")

        # Load base model + LoRA adapters (merged model weights are corrupted)
        base_model = "BioMistral/BioMistral-7B"
        lora_path = "./Final/BioMistral/Narrative/Finetuning/outputs/final_model"
        ...
        if torch.cuda.is_available():
            ...
            print(f"✅ Narrative BioMistral-7B model loaded successfully")
            print(f"   GPU Memory: {allocated:.2f} GB allocated, {reserved:.2f} GB reserved\n")
        else:
            print("✅ Narrative BioMistral-7B model loaded successfully\n")
    ...
    def generate_narrative_summary(self, input_text: str, temperature: float = 0.6,
        ...
        - max_tokens: 768 (allows for summaries up to ~680 words)
        ...
        # This matches the exact format used during training
...
"""
        ...
        generated_raw = self.tokenizer.decode(generated_tokens, skip_special_tokens=True).strip()

        # DEBUG: Always print what model generated
        print(f"\n  📝 RAW MODEL OUTPUT ({len(generated_raw.split())} words):")
        print(f"     First 300 chars: {generated_raw[:300]}")
        if len(generated_raw) > 300:
            print(f"     Last 200 chars: ...{generated_raw[-200:]}")

        # Check if generation was cut off mid-sentence (ends without proper punctuation)
        if generated_raw and not generated_raw[-1] in '.!?':
            # Find last complete sentence
            import re
            sentences = re.split(r'([.!?]\s+)', generated_raw)
            if len(sentences) > 2:  # Has at least one complete sentence
                # Keep only complete sentences
                generated_raw = ''.join(sentences[:-1]).strip()

        # Post-process to clean output
        generated = self._clean_narrative_output(generated_raw)

        # DEBUG: Show what happened during cleaning
        if len(generated) == 0 and len(generated_raw) > 0:
            print(f"  ⚠️  CLEANING REMOVED ALL OUTPUT! Raw had {len(generated_raw.split())} words")
        else:
            print(f"  ✓ After cleaning: {len(generated.split())} words")

        return generated

    def _clean_narrative_output(self, text: str) -> str:
        """Remove any unwanted formatting - RETURN WHATEVER THE MODEL GENERATED"""
        import re
        ...
        # ONLY reject catastrophic repetition (keeps happening with BioMistral)
        ...
        for pattern in repetition_patterns:
            if re.search(pattern, text, flags=re.IGNORECASE):
                ...
                if match and match.start() > 200:
                    ...
                    print(f"  ⚠️  Truncated catastrophic repetition at {match.start()} chars")
                    break
...
def export_for_word_format(results: List[Dict], output_file: str, num_samples: int = None):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("="*100 + "\n")
        f.write("BIOMISTRAL-7B NARRATIVE TEST SET RESULTS\n")
...
def evaluate_test_set(num_samples: int = None):
    """
    Evaluate narrative fine-tuned BioMistral-7B on test set
    ...
    print("="*80)
    print("BIOMISTRAL-7B NARRATIVE TEST SET EVALUATION")
    print("="*80)

    output_dir = "./Final/BioMistral/Narrative/Finetuning/outputs"

    # Load generator
    generator = BioMistralNarrativeGenerator()
    ...
    results_path = f'{output_dir}/biomistral_narrative_results.json'
    with open(results_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"  ✓ Saved: biomistral_narrative_results.json")
    ...
    # Save metrics
    metrics_path = f'{output_dir}/biomistral_narrative_metrics.json'
    with open(metrics_path, 'w') as f:
        json.dump(test_metrics, f, indent=2)
    print(f"  ✓ Saved: biomistral_narrative_metrics.json")
    ...
    word_export_path = os.path.join(output_dir, 'biomistral_narrative_results_for_word.txt')
