#!/usr/bin/env python3
"""
BioMistral-7B Fine-tuning for NARRATIVE Medical Discharge Summaries
Training on 5000 Samples

Fine-tunes BioMistral-7B on NARRATIVE (well-organized paragraph) medical summaries:
...
# Set cache directories to /scratch instead of home directory
...
class BioMistralNarrativeConfig:
    """Configuration for BioMistral-7B fine-tuning on NARRATIVE summaries - 5000 samples"""

    # Model settings
    MODEL_NAME = "BioMistral/BioMistral-7B"  # Medical-pretrained model (not instruction-tuned)
    MAX_SEQ_LENGTH = 4096
    DTYPE = None  # Auto-detect
    LOAD_IN_4BIT = True

    # LoRA settings (optimized for BioMistral-7B)
    LORA_R = 64
    LORA_ALPHA = 64
    LORA_DROPOUT = 0.1
...
def create_narrative_prompt_with_examples(input_text: str, target_text: str = None,
    ...
    """
    Create BioMistral-Instruct format prompt for WELL-ORGANIZED NARRATIVE summary generation
...
"""
    ...
    if target_text:
        ...
        prompt = f"""<s>[INST] You are a medical AI assistant specialized in generating comprehensive narrative discharge summaries. Write well-organized, flowing paragraph summaries without bullet points or section headers.
...
def prepare_dataset(config: BioMistralNarrativeConfig):
    ...
    print("\n✓ Creating BioMistral-format narrative prompts...")
    train_prompts = []
...
def load_model(config: BioMistralNarrativeConfig):
...
def compute_metrics_wrapper(tokenizer):
    ...
    def compute_metrics(eval_pred):
        ...
        for pred, label in zip(decoded_preds, decoded_labels):
            # Find the response section for Phi-3-Medium format
...
def train_model(model, tokenizer, train_dataset, val_dataset, config: BioMistralNarrativeConfig):
    """Train BioMistral-7B model for narrative generation"""
...
def save_training_logs(trainer, config: BioMistralNarrativeConfig, time_callback=None):
...
def save_models(model, tokenizer, config: BioMistralNarrativeConfig):
...
def save_metadata(config: BioMistralNarrativeConfig, train_result, df: pd.DataFrame):
    ...
    metadata_path = f"{config.OUTPUT_DIR}/biomistral_narrative_metadata.json"
...
def main():
    ...
    print("\n" + "="*80)
    print(" "*15 + "BIOMISTRAL-7B NARRATIVE FINE-TUNING")
    print(" "*8 + "Well-Organized Medical Narrative Summaries - 5000 Samples")
    print("="*80)

    # Initialize config
    config = BioMistralNarrativeConfig()
    ...
    create_narrative_training_plots(trainer, config, model_name="BioMistral-7B Narrative", df=df)
    ...
    print(f"   - Metadata: {config.OUTPUT_DIR}/biomistral_narrative_metadata.json")
